...
describe Rack::RequestId do
  ...
  let(:config) { { key: :request_id, value: -> (env) { env['HTTP_X_REQUEST_ID'] }, response_header: 'X-Request-Id' } }
  ...
  describe 'custom middleware configuration' do
    let(:config) { { key: :session_id, value: -> (env) { env['HTTP_X_SESSION_ID'] }, response_header: 'X-Session-Id' } }
